...
def build_curation_tab():
    with gr.Tab("Visualize & Curate"):
        ...
        with gr.Column(visible=False) as dataset_col:
            gr.Markdown("---")
            ...
            with gr.Row(elem_classes=["task-row"]):
                ...
                task_edit_btn = gr.Button("✏️", min_width=50, scale=0, elem_classes=["task-btn"])
                task_save_btn = gr.Button("💾", min_width=50, scale=0, interactive=False, elem_classes=["task-btn"])
    ...
    return dict(
        ...
        push_btn=push_btn, push_status=push_status, push_link_html=push_link_html,
    )
